...
class Payments_model extends CI_Model
{
	...
	public function getSalesReport($stardate='',$enddate='', $firm_id = 0){
			...
			$this->db->select('CO.order_date, CO.order_id, CO.order_type, CO.invoice_number, COP.prod_ref_name, COP.order_qty, COP.order_rate,  P.hsn_code, C.gstin_num, CO.state_code, COP.sgst_per, COP.cgst_per, COP.igst_per');
			$this->db->from('client_orders AS CO');
			$this->db->join('client_order_prodcts AS COP','CO.order_id = COP.order_id');
			...
			/*
			$this->db->select('IP.*, CO.order_id, CO.order_type, CO.order_id, CO.discount, CO.forwardingAmt, CO.order_date, C.comp_name, C.vat_no, C.cst_no, C.gstin_num'); 
			// CO.tax_per, CO.tax_type, CO.excise, Edit by Nikhil on 22-06-2017 01.45 AM 
	...
	public function getInvoiceStockProdcts($invoice_no, $su_id){
		  ...
		  $this->db->select('PS. * , S.supl_comp, S.gstin_num,  POP.prod_ref_name, P.hsn_code, POP.sgst_per, POP.cgst_per, POP.igst_per'); //PBD.tax_per, PBD.tax_type, PBD.excise,
			$this->db->from('product_stock AS PS');
			$this->db->join('product_batch_details AS PBD','PBD.lot_no = PS.lot_no'); 
			$this->db->join('purchase_orders AS PO','PO.purc_order_id = PBD.purc_order_id');
			$this->db->join('suppliers AS S','S.supl_id = PS.su_id');
			$this->db->join('purchase_order_products AS POP','POP.purc_order_id = PO.purc_order_id');
	...
	public function getClientHsnReport($stardate='',$enddate='', $firm_id = 0){
		
		$this->db->select('cop.order_id, p.hsn_code, cop.order_qty, cop.order_rate, cop.sgst_per, cop.cgst_per, cop.igst_per');
		$this->db->from('client_order_prodcts as cop');
		$this->db->join('product as p','cop.order_id = p.pid');

		if($enddate != '' && $stardate !=''){
			$this->db->where("cop.updated_date >=",$stardate);
			$this->db->where("cop.updated_date <=",$enddate);
		}
		
		$this->db->order_by('cop.order_id ASC');
		$query_client_hsn = $this->db->get();
		
		if($query_client_hsn->num_rows()>0)
			{
				return $query_client_hsn->result_array();
			}
			else
			{
				return array();
	...
	public function getPurchaseHsnReport($stardate='',$enddate='', $firm_id = 0){
		$this->db->select('pop.purc_order_id as pid, p.hsn_code, pop.purchase_qty, pop.purchase_rate, pop.sgst_per, pop.cgst_per, pop.igst_per');
		$this->db->from('purchase_order_products as pop');
		$this->db->join('product as p','pop.purc_order_id = p.pid');

		if($enddate != '' && $stardate !=''){
			$this->db->where("pop.updated_date >=",$stardate);
			$this->db->where("pop.updated_date <=",$enddate);
		}
		$this->db->order_by('pop.purc_order_id ASC');
		$query_client_hsn = $this->db->get();
		//echo $this->db->last_query();die; 
		if($query_client_hsn->num_rows()>0)
			{
