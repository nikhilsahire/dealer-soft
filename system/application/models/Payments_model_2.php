...
class Payments_model extends CI_Model
{
	...
	public function getAllClientPaymentList($stardate='',$enddate='', $firm_id = 0){
	
		  $this->db->select('IP.*, C.comp_name, U.first_name, U.last_name');
			...
			if($enddate != '' && $stardate !=''){
				$this->db->where("IP.updated_date >=",$stardate);  
				$this->db->where("IP.updated_date <=",$enddate);
			}
	...
	public function getOrderAmount($order_id)
	 {
		 	$this->db->select("SUM(COP.`order_qty` * COP.`order_rate`) AS orderTotal, SUM(COP.`order_rate` * COP.`dispatch_qty` ) AS dispatchAmt");
	...
	/* Get order products based on order id */
	 ...
	 public function getOrderProducts($order_id){
	 		...
	 		$this->db->select("COP.*, P.product_name, P.prod_unit");
			$this->db->from("client_order_prodcts AS COP");
	...
	/* Get the list of the firm */
	 ...
	 public function getFirmRow($firm_id, $field_name){
	 
	 		$this->db->select("(CF.".$field_name.")+1 AS ".$field_name.",  CF.firm_code,  CF.current_year");
			$this->db->from("company_firms AS CF");
	 ...
	 */ 
	  ...
	  public function addOrder($data_order,$data_order_prod)
	 {
		 ...
		 $firmRow = $this->getFirmRow($data_order['invoice_firm'], 'order_id');
		 ...
		 for($i=0; $i < $totalRows; $i++){
		 	if($data_order_prod['pid'][$i] != '' && $data_order_prod['order_rate'][$i] != '' && $data_order_prod['order_qty'][$i] != ''){
				...
				$order_prod['order_packing'] = round($data_order_prod['packing'][$i],2);
	 ...
	 */
	 ...
	 public function createChallanBKP_03_02_2016($data_order,$order_challan_shipping){
	 	if(sizeof($data_order) > 0){
		    // get the challan number
			$firmRow = $this->getFirmRow($order_challan_shipping['firm_id'], 'current_challan');
			$challan_number = $firmRow[0]['firm_code'].'/'.$firmRow[0]['current_year'].'/'.$firmRow[0]['current_challan'];
			// get the Invoiice  number
			$firmInvoiceRow = $this->getFirmRow($order_challan_shipping['firm_id'], 'current_invoice');
			$invoice_number = $firmInvoiceRow[0]['firm_code'].'/'.$firmInvoiceRow[0]['current_year'].'/'.$firmInvoiceRow[0]['current_invoice'];
			// update the order id field in firm table			
			$data_firm['current_challan'] = $firmRow[0]['current_challan'];
			$data_firm['current_invoice'] = $firmInvoiceRow[0]['current_invoice'];
			$this->db->where("firm_id", $order_challan_shipping['firm_id']);
			$this->db->update("company_firms", $data_firm); 
		  	// add notification
				...
				$today = date("Y-m-d");
				$notificationArray['uid'] = $this->session->userdata('userid');
				$notificationArray['message_text'] = 'Take follow up of order # '.$challan_number;
				$notificationArray['added_on'] = $today;
				$notificationArray['reminder_date'] = date('Y-m-d',strtotime($today."+ 8 days"));
				$notificationArray['read_flag'] = 'Pending';
				$this->add_notification($notificationArray);
	...
	/* Get challan details based on the challan number */
	 public function getChallanDetails($challan_number, $order_id){
	 		...
	 		$this->db->select("OC.`challan_no`, OC.`chalan_date`,OC.`invoice_no`, OC.`invoice_date`, OC.`notification_id`, NF.`reminder_date` , CO.shipping_address,CO.order_type, C.comp_name, C.vat_no, C.cst_no, CF.firm_name, CF.vat_num, CF.cst_num, CO.po_ref, CO.po_date, CO.order_id");
	 ...
	 */ 
	 ...
	 public function updateInvoicePayments($invoiceArray,$challan_no,$insertPayment){
	 	    if(sizeof($invoiceArray) > 0 ){
				...
				$notificationArray['message_text'] = 'Your payment is due on '.$insertPayment['reminder_date'].' for invoice # '.$insertPayment['invoice_number'] ;
				$notificationArray['added_on'] = $today;
